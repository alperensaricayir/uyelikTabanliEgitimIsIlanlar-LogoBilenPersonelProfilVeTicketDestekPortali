<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center flex-wrap gap-4">
            <h2 class="font-semibold text-xl text-neutral-900 leading-tight flex items-center gap-3">
                ...
                @endphp
                <span class="inline-flex px-2.5 py-1 rounded-md text-[11px] font-bold uppercase {{ $badgeCls }}">
                    {{ $badgeLabel }}
                </span>
            </h2>
    ...
    <div class="py-8 bg-neutral-50 min-h-screen">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                @foreach($ticket->replies as $reply)
                    ...
                    <div class="flex gap-4 {{ $isMe ? 'flex-row-reverse' : '' }}">
                        <div class="flex-shrink-0">
                            <div
                                class="w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold {{ $isAgent ? 'bg-violet-600 text-white' : 'bg-neutral-200 text-neutral-700' }}">
                                {{ strtoupper(substr($reply->user->name ?? 'U', 0, 1)) }}
                            </div>
                        </div>
                        <div class="flex flex-col {{ $isMe ? 'items-end' : 'items-start' }} max-w-[85%]">
                            <span class="text-xs font-semibold text-neutral-500 mb-1 mx-1">
                                {{ $reply->user->name ?? 'Bilinmeyen' }}
                                @if($isAgent) <span class="text-violet-600">(Destek)</span> @endif
                                • {{ $reply->created_at->format('d M Y H:i') }}
                            </span>
                            <div
                                class="bg-white border text-sm text-neutral-800 p-4 shadow-sm whitespace-pre-wrap {{ $isMe ? 'rounded-2xl rounded-tr-sm border-neutral-200' : 'rounded-2xl rounded-tl-sm border-violet-100 ring-1 ring-violet-50' }}">
                                        ...
                                        @foreach($reply->attachments as $attachment)
                                            <a href="{{ Storage::disk('public')->url($attachment->file_path) }}" target="_blank"
                                                ...
                                                <img src="{{ Storage::disk('public')->url($attachment->file_path) }}" alt="Ek"
                                                    class="max-w-[200px] max-h-[200px] object-cover rounded-xl border border-neutral-200 hover:opacity-90 transition-opacity">
                                            </a>
                                        @endforeach
                                    </div>
                                @endif
            ...
            {{-- Reply Form --}}
            <div class="pt-4 border-t border-neutral-200 mt-8">
                @if($ticket->status !== 'closed')
                    <div class="bg-white border border-neutral-200 shadow-sm rounded-2xl p-6">
                        <h3 class="font-semibold text-neutral-900 mb-4">Yeni Yanıt Ekle</h3>
                        <form method="POST" action="{{ route('tickets.reply', $ticket) }}" enctype="multipart/form-data">
                            ...
                            <div class="mb-4">
                                <label class="block text-sm font-semibold text-neutral-900 mb-1.5">Ekler (Görsel, Max:
                                    5MB)</label>
                                <input type="file" name="images[]" multiple
                                    accept="image/jpeg,image/png,image/webp,image/jpg"
                                    class="block w-full text-sm text-neutral-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-violet-50 file:text-violet-700 hover:file:bg-violet-100 transition-colors cursor-pointer border border-neutral-200 rounded-lg">
                                @error('images.*')
                                    <p class="text-rose-500 text-xs mt-1.5 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                ...
                @else
                    <div
                        class="bg-neutral-100 border border-neutral-200 text-neutral-500 rounded-2xl p-6 text-center text-sm font-medium">
