    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            @if(isset($isPreview) && $isPreview)
                <div
                    class="mb-6 bg-amber-100 dark:bg-amber-900/30 border border-amber-400 dark:border-amber-800 text-amber-700 dark:text-amber-400 px-4 py-3 rounded-lg flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <strong>Önizleme Modu:</strong> Profiliniz şu anda Gizli. Ancak kendi profiliniz olduğu için
                        ziyaretçilerin nasıl göreceğini (eğer herkese açık yapsaydınız) önizliyorsunuz.
                    </div>
                </div>
            @endif
            ...
            <div
                ...
                <div class="flex flex-col md:flex-row md:items-start justify-between gap-6">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        @if(!$isOwner)
                            <form action="{{ route('profile.like', $user) }}" method="POST" class="inline">
                                @csrf
                                @php
                                    $hasLiked = auth()->check() ? \App\Models\Like::where('user_id', auth()->id())->where('likeable_type', \App\Models\User::class)->where('likeable_id', $user->id)->exists() : false;
                                @endphp
                                <button type="submit"
                                    class="px-5 py-2.5 bg-white dark:bg-gray-800 border {{ $hasLiked ? 'border-pink-500 text-pink-600 dark:text-pink-400' : 'border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300' }} hover:bg-gray-50 dark:hover:bg-gray-700 text-sm font-medium rounded-lg transition shadow-sm flex items-center gap-2">
                                    <svg class="w-5 h-5 {{ $hasLiked ? 'fill-current' : 'fill-none stroke-current' }}"
                                        viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                        <path
                                            d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z">
                                        </path>
                                    </svg>
                                    Beğen ({{ $user->likes_count }})
                                </button>
                            </form>
                        @endif
                    </div>
                    ...
                    @php
                        $links = [
                            [
                                ...
                                'label' => 'Website',
                                'icon' => '<svg class="w-5 h-5 align-middle"
