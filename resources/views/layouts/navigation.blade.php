<nav x-data="{ open: false, showSearch: false, premiumModalOpen: false }"
            ...
            <div class="flex-shrink-0 flex items-center gap-2 pl-4 py-2">
                <a href="{{ route('dashboard') }}" class="flex items-center gap-2 group">
                    <img src="{{ asset('images/3sgruplogoyeni.png') }}"
                        class="h-14 sm:h-16 md:h-20 lg:h-24 w-auto object-contain hover:opacity-80 transition-opacity drop-shadow-sm dark:drop-shadow-md"
                        alt="3S Grup Logo"
                        onerror="this.outerHTML='<span class=\'text-2xl md:text-3xl font-bold font-serif text-gray-900 dark:text-white\'>3S Grup</span>'">
                </a>
            </div>
            ...
            <div class="hidden md:flex items-center gap-1 flex-1 justify-center">
                @php
                    $navLinks = [
                        ...
                        ['route' => 'products.index', 'label' => 'Ürünlerimiz', 'icon' => '🛒'],
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
                ...
                @else
                    <div class="hidden md:flex items-center gap-3">
                        <a href="{{ route('login') }}"
                            class="text-sm font-medium text-neutral-600 hover:text-neutral-900 dark:text-gray-300 dark:hover:text-white transition-colors">Giriş
                            Yap</a>
                        <a href="{{ route('register') }}"
                            class="text-sm font-medium bg-violet-600 hover:bg-violet-700 text-white px-4 py-2 rounded-lg transition-colors shadow-sm">Kayıt
                            Ol</a>
                    </div>
                @endauth
    ...
    <div x-show="open" x-transition
        ...
        <div class="px-2 pt-2 pb-3 space-y-1">
            ...
            <a href="{{ route('dashboard') }}"
                class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('dashboard') ? 'bg-violet-50 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300' : 'text-neutral-600 dark:text-gray-300 hover:text-neutral-900 hover:bg-neutral-50 dark:hover:text-white dark:hover:bg-gray-800' }}">Ana
                Sayfa</a>
            <a href="{{ route('trainings.index') }}"
                class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('trainings.*') ? 'bg-violet-50 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300' : 'text-neutral-600 dark:text-gray-300 hover:text-neutral-900 hover:bg-neutral-50 dark:hover:text-white dark:hover:bg-gray-800' }}">Eğitimler</a>
            <a href="{{ route('discover.index') }}"
                class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('discover.*') ? 'bg-violet-50 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300' : 'text-neutral-600 dark:text-gray-300 hover:text-neutral-900 hover:bg-neutral-50 dark:hover:text-white dark:hover:bg-gray-800' }}">Keşfet</a>
            <a href="{{ route('jobs.index') }}"
                class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('jobs.*') ? 'bg-violet-50 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300' : 'text-neutral-600 dark:text-gray-300 hover:text-neutral-900 hover:bg-neutral-50 dark:hover:text-white dark:hover:bg-gray-800' }}">İş
                İlanları</a>
            <a href="{{ route('products.index') }}"
                class="block px-3 py-2 rounded-md text-base font-medium {{ request()->routeIs('products.*') ? 'bg-violet-50 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300' : 'text-neutral-600 dark:text-gray-300 hover:text-neutral-900 hover:bg-neutral-50 dark:hover:text-white dark:hover:bg-gray-800' }}">Ürünlerimiz</a>
            <a href="{{ route('tickets.index') }}"
                class="block px-3 py-2 rounded-md text-base font-medium {{ request()->is('tickets*') ? 'bg-violet-50 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300' : 'text-neutral-600 dark:text-gray-300 hover:text-neutral-900 hover:bg-neutral-50 dark:hover:text-white dark:hover:bg-gray-800' }}">
                Destek Ticketları
                @auth()->check())
                    @php
                        $openTickets = auth()->user()->tickets()->openStatus()->count();
            ...
            @auth
                ...
                @if(auth()->user()->role === 'admin')
                    <a href="/admin"
                        class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-neutral-700 hover:bg-neutral-100">⚙️
                        Admin Panel</a>
                @endif
                @if(auth()->user()->isPremium())
                    <button type="button" @click="premiumModalOpen = true"
            ...
            @else
                <div class="border-t border-neutral-100 dark:border-gray-800 pt-2 mt-2 space-y-1">
                    <a href="{{ route('login') }}"
                        class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-neutral-700 dark:text-gray-300 hover:bg-neutral-100 dark:hover:bg-gray-800">
                        🔑 Giriş Yap
                    </a>
                    <a href="{{ route('register') }}"
                        class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-violet-700 dark:text-violet-400 bg-violet-50 dark:bg-violet-900/30 hover:bg-violet-100 dark:hover:bg-violet-900/50">
                        ✨ Kayıt Ol
                    </a>
                </div>
            @endauth
